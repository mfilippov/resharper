...
namespace ReSharper.Nuke.Analysis
{
    ...
    public class NukeTargetPropertyAnalyzer : NukeElementProblemAnalyzer<IMemberOwnerDeclaration>
    {
        protected override void Analyze(IMemberOwnerDeclaration element, ElementProblemAnalyzerData data, IHighlightingConsumer consumer)
        {
            ...
            {
                if (typeMember.IsNukeBuildTarget())
                {
        ...
        {
            foreach (var declaration in property.GetDeclarations()) AddGutterMark(declaration, consumer);
        }
